...
def arduino_simulaattori(lat_multi, lon_multi, jono):
    ...
    #odottaa GPS yhteyttä
    searching = True
    while True:
        serdata = ser.readline()
        if serdata[:6] == b'$GPGLL':
            #print(serdata.decode()[:-2])
            dataosa = serdata.decode()[6:-2]
            for i in range(len(dataosa)-1):
                if (dataosa[i] == ",") and (dataosa[i+1] == ","):
                    print( "$GPGLL" + str(dataosa), "  /NoGoodData")
                    break
                else:
                    try:
                        asd = "$GPGLL" + str(dataosa)
                        latlon = asd.split(",")
                        varygeo_lat = muunnin(latlon[1])
                        varygeo_lon = muunnin(latlon[3])
                        ...
                        searching = True
                        print("DRAMAATTISESTI VITUIKS")
                        quit()
                    break
        if serdata[:6] == b'$GPGSV' and searching:
            print(serdata[:8].decode())
